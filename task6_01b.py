...
class TrafficLight:
    ...
    __modes = (("red", 7), ("yellow", 1), ("green", 5), ("yellow", 1))
    __light_start = 0
    __next_light = 0
    ...
    def __tic(self):
        '''
        В зависимости от прошедшего времени переключается цвет светофора,
        при этом не предполагается использование блокировки sleep.
        :return: str
        '''
        if self.__light_start + dict(self.__modes)[
            self.__color] <= time.time():
            self.__color, self.__light_start = (
                self.__modes[self.__next_light][0],
                time.time(),
            )
            self.__next_light = (
                self.__next_light + 1
